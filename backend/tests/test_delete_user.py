...
def test_super_admin_elimina_usuario(client, db):
    ...
    r = client.delete(f"/auth/users/{victima.id}", headers=h)
...
def test_no_elimina_self(client, db):
    ...
    h = _hdr(client, "super@test.ec")
    r = client.delete(f"/auth/users/{me.id}", headers=h)
...
def test_admin_tenant_no_elimina_otro_tenant(client, db):
    ...
    assert client.delete(f"/auth/users/{otro.id}", headers=h).status_code == 403
...
def test_no_elimina_ultimo_super_admin(client, db):
    ...
    assert client.delete(f"/auth/users/{otro_super.id}", headers=h).status_code == 200
